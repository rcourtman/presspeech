...
release-script self-test, model-manifest
...
Keep the two-minute recording safety limit epoch-guarded and cancel its timer
when capture stops; a stale timer must never stop a newer recording.
...
`windows/build-release.ps1`, `windows/validate-release-context.ps1`, and
`windows/validate-release-tag.ps1`. The release build is intentionally one-folder
